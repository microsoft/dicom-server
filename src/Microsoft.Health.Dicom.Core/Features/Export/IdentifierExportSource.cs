...
internal sealed class IdentifierExportSource : IExportSource
{
    ...
    public async IAsyncEnumerator<ReadResult> GetAsyncEnumerator(CancellationToken cancellationToken = default)
    {
        ...
        foreach (DicomIdentifier identifier in _identifiers)
        {
            ...
            {
                ResourceType.Study => await _instanceStore.GetInstanceIdentifiersInStudyAsync(_partition.PartitionKey, _partition.PartitionName, identifier.StudyInstanceUid, cancellationToken),
                ResourceType.Series => await _instanceStore.GetInstanceIdentifiersInSeriesAsync(_partition.PartitionKey, _partition.PartitionName, identifier.StudyInstanceUid, identifier.SeriesInstanceUid, cancellationToken),
                _ => await _instanceStore.GetInstanceIdentifierAsync(_partition.PartitionKey, _partition.PartitionName, identifier.StudyInstanceUid, identifier.SeriesInstanceUid, identifier.SopInstanceUid, cancellationToken),
            };
