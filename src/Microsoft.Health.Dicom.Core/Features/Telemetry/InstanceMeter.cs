...
public sealed class InstanceMeter : IDisposable
{
    ...
    public InstanceMeter()
    {
        ...
        InstanceCount = _meter.CreateCounter<long>(nameof(InstanceCount), "count", "Total number of instances processed");
        RetrieveInstanceCount = _meter.CreateCounter<long>(nameof(InstanceCount), "count", "Total number of instances retrieved");
        ...
        CountDeletionsMaxRetry = _meter.CreateCounter<long>(nameof(CountDeletionsMaxRetry), "count", "Number of exhausted instance deletion attempts");
