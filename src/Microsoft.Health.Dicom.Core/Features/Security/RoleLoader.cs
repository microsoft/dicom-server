    ...
    public class RoleLoader : IHostedService
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            validatingReader.ValidationEventHandler += (sender, args) =>
                throw new InvalidDefinitionException(string.Format(DicomCoreResource.ErrorValidatingRoles, args.Message));
            ...
            foreach (IGrouping<string, Role> grouping in _authorizationConfiguration.Roles.GroupBy(r => r.Name))
            {
                if (grouping.Count() > 1)
                {
                    throw new InvalidDefinitionException(
                        string.Format(CultureInfo.CurrentCulture, DicomCoreResource.DuplicateRoleNames, grouping.Count(), grouping.Key));
                }
            }
