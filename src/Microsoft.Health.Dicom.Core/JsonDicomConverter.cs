...
// <auto-generated /> TODO THIS CODE SHOULD BE CONSUMED FROM FO-DICOM NUGET WHEN THEY MERGE THIS PR
// https://github.com/fo-dicom/fo-dicom/pull/788
using System;
...
namespace Dicom.Serialization
{
    ...
    public class JsonDicomConverter : JsonConverter
    {
        ...
        public JsonDicomConverter(bool writeTagsAsKeywords = true)
        {
        ...
        {
            // Note that DicomElement.Get<> first uses decimal.Parse that seems to deal with these cases.
            // With .Select(x => decimal.Parse(x, NumberStyles.Any, CultureInfo.InvariantCulture)
            // It would be better to refactor the following to use similar methods with a common test base.
            ...
            // Add a leading zero if first char is decimal point
            if (val.Length > 1 && val[0] == '.')
            {
                val = "0" + val;
            }

            // Add a trailing 0 if last char is decimal point
            if (val.Length > 1 && val.Last() == '.')
            {
                val = val + "0";
            }
        ...
        private static void WriteJsonElement<T>(JsonWriter writer, DicomElement elem)
        {
            if (elem.Count != 0)
            {
                T[] deserializedValues = null;

                // Not all DICOM files contain valid values. We try here and silently ignore the
                // tag if it cannot be parsed. 
                try
                {
                    deserializedValues = elem.Get<T[]>();
                }
                catch (FormatException)
                {
                    // TODO: We should report this as a removed element in the DICOM web STOW-RS response?
                }
        ...
        private void WriteJsonOther(JsonWriter writer, DicomElement elem)
        {
