...
namespace Microsoft.Health.Dicom.Core.UnitTests.Features.Query
{
    public class QueryParserTests
    {
        ...
        public void GivenStudyDate_WithValidRangeMatch_CheckCondition(string key, string value)
        {
            ...
            Assert.True(cond.Minimum == DateTime.ParseExact(value.Split('-')[0], QueryFilterConditionParser.DateTagValueFormat, null));
            Assert.True(cond.Maximum == DateTime.ParseExact(value.Split('-')[1], QueryFilterConditionParser.DateTagValueFormat, null));
        }
