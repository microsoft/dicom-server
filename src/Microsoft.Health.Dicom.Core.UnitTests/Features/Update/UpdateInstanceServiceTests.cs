...
public class UpdateInstanceServiceTests
{
    ...
    public async Task GivenDatasetToUpdateIsNull_WhenCalled_ThrowsArgumentNullException()
    {
        ...
        await Assert.ThrowsAsync<ArgumentNullException>(() =>
            _updateInstanceService.UpdateInstanceBlobAsync(instanceFileIdentifier, PartitionKey, null, CancellationToken.None));
    }
    ...
    public async Task GivenInstanceFileIdentifierIsNull_WhenCalled_ThrowsArgumentNullException()
    {
        ...
        await Assert.ThrowsAsync<ArgumentNullException>(() =>
            _updateInstanceService.UpdateInstanceBlobAsync(null, PartitionKey, datasetToUpdate, CancellationToken.None));
    }
    ...
    public async Task GivenNewVersionIsNull_WhenCalled_ThrowsArgumentException()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() =>
            _updateInstanceService.UpdateInstanceBlobAsync(instanceFileIdentifier, PartitionKey, datasetToUpdate, CancellationToken.None));
    }
    ...
    public async Task GivenValidInput_WhenDeletingBothFileAndMetadata_ThenItDeletes()
    {
        ...
        await _updateInstanceService.DeleteInstanceBlobAsync(fileIdentifier, PartitionKey);
        await _fileStore.Received(1).DeleteFileIfExistsAsync(fileIdentifier, PartitionKey, CancellationToken.None);
    ...
    public async Task GivenValidInput_WhenCallingUpdateBlobAsync_ShouldCallUpdateInstanceFileAsync_AndUpdateInstanceMetadataAsync()
    {
        ...
        _fileStore.GetFileAsync(fileIdentifier, PartitionKey, cancellationToken).Returns(streamAndStoredFile.Value);
        ...
        _fileStore.GetFileContentInRangeAsync(newFileIdentifier, PartitionKey, Arg.Any<FrameRange>(), cancellationToken).Returns(binaryData);
        _fileStore.UpdateFileBlockAsync(newFileIdentifier, PartitionKey, Arg.Any<string>(), copyStream, cancellationToken).Returns(Task.CompletedTask);
        ...
        await _updateInstanceService.UpdateInstanceBlobAsync(instanceFileIdentifier, PartitionKey, datasetToUpdate, cancellationToken);

        await _fileStore.DidNotReceive().CopyFileAsync(fileIdentifier, newFileIdentifier, PartitionKey, cancellationToken);
        await _fileStore.Received(1).GetFileAsync(fileIdentifier, PartitionKey, cancellationToken);
        ...
        await _fileStore.Received(1).GetFileContentInRangeAsync(newFileIdentifier, PartitionKey, Arg.Any<FrameRange>(), cancellationToken);
        await _fileStore.Received(1).UpdateFileBlockAsync(newFileIdentifier, PartitionKey, Arg.Any<string>(), Arg.Any<Stream>(), cancellationToken);
    ...
    public async Task GivenValidInputWithLargeFile_WhenCallingUpdateBlobAsync_ShouldCallUpdateInstanceFileAsync_AndUpdateInstanceMetadataAsync()
    {
        ...
        await _metadataStore.Received(1).GetInstanceMetadataAsync(fileIdentifier, cancellationToken);
        await _metadataStore.Received(1).StoreInstanceMetadataAsync(streamAndStoredFile.Key.Dataset, newFileIdentifier, cancellationToken);
        await _fileStore.Received(1).GetFileContentInRangeAsync(newFileIdentifier, PartitionKey, Arg.Is<FrameRange>(x => x.Length == firstBlockLength), cancellationToken);
        await _fileStore.Received(1).UpdateFileBlockAsync(newFileIdentifier, PartitionKey, Arg.Any<string>(), Arg.Any<Stream>(), cancellationToken);
        await _fileStore.Received(1).StoreFileInBlocksAsync(
            newFileIdentifier,
            PartitionKey,
    ...
    public async Task GivenValidInputWithExistingFile_WhenCallingUpdateBlobAsync_ShouldCallUpdateInstanceFileAsync_AndUpdateInstanceMetadataAsync()
    {
        ...
        _fileStore.CopyFileAsync(fileIdentifier, newFileIdentifier, PartitionKey, cancellationToken).Returns(Task.CompletedTask);
        _fileStore.GetFileAsync(fileIdentifier, PartitionKey, cancellationToken).Returns(streamAndStoredFile.Value);
        _metadataStore.GetInstanceMetadataAsync(fileIdentifier, cancellationToken).Returns(streamAndStoredFile.Key.Dataset);
        _metadataStore.StoreInstanceMetadataAsync(streamAndStoredFile.Key.Dataset, newFileIdentifier, cancellationToken).Returns(Task.CompletedTask);
        _fileStore.GetFileContentInRangeAsync(newFileIdentifier, PartitionKey, Arg.Any<FrameRange>(), cancellationToken).Returns(binaryData);
        _fileStore.UpdateFileBlockAsync(newFileIdentifier, PartitionKey, Arg.Any<string>(), copyStream, cancellationToken).Returns(Task.CompletedTask);
        _fileStore.GetFirstBlockPropertyAsync(newFileIdentifier, PartitionKey, cancellationToken).Returns(firstBlock);
        ...
        await _fileStore.Received(1).CopyFileAsync(fileIdentifier, newFileIdentifier, PartitionKey, cancellationToken);
        await _fileStore.DidNotReceive().GetFileAsync(fileIdentifier, PartitionKey, cancellationToken);
        await _metadataStore.Received(1).GetInstanceMetadataAsync(fileIdentifier, cancellationToken);
        await _metadataStore.Received(1).StoreInstanceMetadataAsync(streamAndStoredFile.Key.Dataset, newFileIdentifier, cancellationToken);
        await _fileStore.Received(1).GetFileContentInRangeAsync(newFileIdentifier, PartitionKey, Arg.Is<FrameRange>(x => x.Length == firstBlockLength), cancellationToken);
        await _fileStore.Received(1).UpdateFileBlockAsync(newFileIdentifier, PartitionKey, Arg.Any<string>(), Arg.Any<Stream>(), cancellationToken);
        await _fileStore.DidNotReceive().StoreFileInBlocksAsync(
            ...
            Arg.Is<IDictionary<string, long>>(x => x.Count == 2 && x.Sum(y => y.Value) == copyStream.Length),
            cancellationToken);
        await _fileStore.Received(1).GetFirstBlockPropertyAsync(newFileIdentifier, PartitionKey, cancellationToken);
