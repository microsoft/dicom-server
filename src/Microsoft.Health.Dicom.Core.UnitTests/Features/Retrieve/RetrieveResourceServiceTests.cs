...
public class RetrieveResourceServiceTests
{
    ...
    public async Task GivenStoredInstancesWhereOneIsMissingFile_WhenRetrieveRequestForStudy_ThenNotFoundIsThrown()
    {
        ...
        _fileStore.GetFilePropertiesAsync(Arg.Any<long>(), DefaultPartition.Key, DefaultCancellationToken).Returns(new FileProperties { ContentLength = 1000 });
        ...
        for (int i = 0; i < instances.Count - 1; i++)
        {
            ...
            _fileStore.GetStreamingFileAsync(instance.VersionedInstanceIdentifier.Version, DefaultPartition.Key, DefaultCancellationToken).Returns(stream.Value);
        ...
        _fileStore.GetStreamingFileAsync(instances.Last().VersionedInstanceIdentifier.Version, DefaultPartition.Key, DefaultCancellationToken).Throws(new InstanceNotFoundException());
    ...
    public async Task GivenStoredInstances_WhenRetrieveRequestForStudy_ThenInstancesInStudyAreRetrievedSuccesfully()
    {
        ...
        {
            _fileStore.GetStreamingFileAsync(count, DefaultPartition.Key, DefaultCancellationToken).Returns(file.Value);
            count++;
        }
    ...
    public async Task GivenStoredInstancesWithOriginalVersion_WhenRetrieveRequestForStudyForOriginal_ThenInstancesInStudyAreRetrievedSuccesfully()
    {
        ...
        await _fileStore
            .Received(3)
            .GetFilePropertiesAsync(Arg.Is<long>(x => x == 5), DefaultPartition.Key, DefaultCancellationToken);

        await _fileStore
            .Received(3)
            .GetStreamingFileAsync(Arg.Is<long>(x => x == 5), DefaultPartition.Key, DefaultCancellationToken);
    }
    ...
    public async Task GivenStoredInstancesWithOriginalVersion_WhenRetrieveRequestForStudyForLatest_ThenInstancesInStudyAreRetrievedSuccesfully()
    {
        ...
        await _fileStore
            .Received(1)
            .GetFilePropertiesAsync(Arg.Is<long>(x => x == 0), DefaultPartition.Key, DefaultCancellationToken);

        await _fileStore
            .Received(1)
            .GetStreamingFileAsync(Arg.Is<long>(x => x == 0), DefaultPartition.Key, DefaultCancellationToken);
    }
    ...
    public async Task GivenInstancesWithOriginalVersion_WhenRetrieveRequestForStudyForOriginalWithTranscoding_ThenInstancesAreReturned()
    {
        ...
        await _fileStore
            .DidNotReceive()
            .GetStreamingFileAsync(Arg.Any<long>(), DefaultPartition.Key, DefaultCancellationToken);

        await _fileStore
            .Received(1)
            .GetFileAsync(Arg.Is<long>(x => x == 5), DefaultPartition.Key, DefaultCancellationToken);
    }
    ...
    public async Task GivenSpecificTransferSyntax_WhenRetrieveRequestForStudy_ThenInstancesInStudyAreTranscodedSuccesfully()
    {
        ...
        _fileStore.GetFileAsync(0, DefaultPartition.Key, DefaultCancellationToken).Returns(streamsAndStoredFile.Value);
        _fileStore.GetFilePropertiesAsync(instanceMetadata.VersionedInstanceIdentifier.Version, DefaultPartition.Key, DefaultCancellationToken).Returns(new FileProperties { ContentLength = streamsAndStoredFile.Value.Length });
    ...
    public async Task GivenStoredInstancesWhereOneIsMissingFile_WhenRetrieveRequestForSeries_ThenNotFoundIsThrown()
    {
        ...
        // For the last identifier, set up the fileStore to throw a store exception with the status code 404 (NotFound).
        _fileStore.GetStreamingFileAsync(versionedInstanceIdentifiers.Last().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.Last().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Throws(new InstanceNotFoundException());
    ...
    public async Task GivenStoredInstancesWithMissingFile_WhenRetrieveRequestForInstance_ThenNotFoundIsThrown()
    {
        ...
        _fileStore.GetStreamingFileAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Throws(new InstanceNotFoundException());
    ...
    public async Task GivenStoredInstances_WhenRetrieveRequestForInstance_ThenInstanceIsRetrievedSuccessfully(PayloadTypes payloadTypes)
    {
        ...
        _fileStore.GetStreamingFileAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(streamAndStoredFile.Value);
    ...
    public async Task GivenStoredInstancesWithoutFrames_WhenRetrieveRequestForFrame_ThenNotFoundIsThrown()
    {
        ...
        _fileStore.GetFileAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(streamOfStoredFiles);
        _fileStore.GetFilePropertiesAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(new FileProperties { ContentLength = streamOfStoredFiles.Length });
    ...
    public async Task GivenStoredInstancesWithFrames_WhenRetrieveRequestForFrames_ThenFramesInInstanceAreRetrievedSuccesfully()
    {
        ...
        _fileStore.GetFileAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(streamAndStoredFile.Value);
        _fileStore.GetFilePropertiesAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(new FileProperties { ContentLength = streamAndStoredFile.Value.Length });
    ...
    public async Task GetInstances_WithAcceptType_ThenResponseContentTypeIsCorrect(string requestedTransferSyntax, string originalTransferSyntax, string expectedTransferSyntax)
    {
        ...
        streamsAndStoredFiles.ForEach(x => _fileStore.GetFileAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(x.Value));
        streamsAndStoredFiles.ForEach(x => _fileStore.GetStreamingFileAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(x.Value));
        _fileStore.GetFilePropertiesAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(new FileProperties { ContentLength = streamsAndStoredFiles.First().Value.Length });
    ...
    public async Task GetInstances_WithAcceptTypeOnOldFile_ThenResponseContentTypeWithBackCompatWorks(string requestedTransferSyntax, string originalTransferSyntax, string expectedTransferSyntax)
    {
        ...
        // For each instance identifier, set up the fileStore to return a stream containing a file associated with the identifier.
        var streamsAndStoredFiles = versionedInstanceIdentifiers.Select(x => RetrieveHelpers.StreamAndStoredFileFromDataset(RetrieveHelpers.GenerateDatasetsFromIdentifiers(x.VersionedInstanceIdentifier, originalTransferSyntax), _recyclableMemoryStreamManager).Result).ToList();
        streamsAndStoredFiles.ForEach(x => _fileStore.GetFileAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(x.Value));
        streamsAndStoredFiles.ForEach(x => _fileStore.GetStreamingFileAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(x.Value));
        _fileStore.GetFilePropertiesAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(new FileProperties { ContentLength = streamsAndStoredFiles.First().Value.Length });
        _fileStore.GetFilePropertiesAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(new FileProperties { ContentLength = streamsAndStoredFiles.First().Value.Length });
    ...
    public async Task GetFrames_WithLargeFileSize_ThrowsNotSupported()
    {
        ...
        _fileStore.GetFilePropertiesAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.PartitionKey, DefaultCancellationToken).Returns(new FileProperties { ContentLength = aboveMaxFileSize });
    ...
    private List<InstanceMetadata> SetupInstanceIdentifiersList(ResourceType resourceType, int partitionKey = DefaultPartition.Key, InstanceProperties instanceProperty = null)
    {
        ...
        switch (resourceType)
        {
            ...
            case ResourceType.Frames:
                ...
                _instanceStore.GetInstanceIdentifierWithPropertiesAsync(partitionKey, _studyInstanceUid, _firstSeriesInstanceUid, _sopInstanceUid, DefaultCancellationToken).Returns(dicomInstanceIdentifiersList.SkipLast(1).ToList());
