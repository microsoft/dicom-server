...
public class RetrieveResourceServiceTests
{
    ...
    public async Task GivenStoredInstancesWhereOneIsMissingFile_WhenRetrieveRequestForStudy_ThenNotFoundIsThrown()
    {
        ...
        _fileStore.GetFilePropertiesAsync(Arg.Any<long>(), Partition.Default.Name, DefaultCancellationToken).Returns(new FileProperties { ContentLength = 1000 });
    ...
    public async Task GivenStoredInstances_WhenRetrieveRequestForStudy_ThenInstancesInStudyAreRetrievedSuccesfully()
    {
        ...
        {
            _fileStore.GetStreamingFileAsync(count, Partition.Default.Name, DefaultCancellationToken).Returns(file.Value);
            count++;
        }
    ...
    public async Task GivenStoredInstancesWithOriginalVersion_WhenRetrieveRequestForStudyForOriginal_ThenInstancesInStudyAreRetrievedSuccesfully()
    {
        ...
        await _fileStore
            .Received(3)
            .GetFilePropertiesAsync(Arg.Is<long>(x => x == 5), Partition.Default.Name, DefaultCancellationToken);

        await _fileStore
            .Received(3)
            .GetStreamingFileAsync(Arg.Is<long>(x => x == 5), Partition.Default.Name, DefaultCancellationToken);
    }
    ...
    public async Task GivenStoredInstancesWithOriginalVersion_WhenRetrieveRequestForStudyForLatest_ThenInstancesInStudyAreRetrievedSuccesfully()
    {
        ...
        await _fileStore
            .Received(1)
            .GetFilePropertiesAsync(Arg.Is<long>(x => x == 0), Partition.Default.Name, DefaultCancellationToken);

        await _fileStore
            .Received(1)
            .GetStreamingFileAsync(Arg.Is<long>(x => x == 0), Partition.Default.Name, DefaultCancellationToken);
    }
    ...
    public async Task GivenInstancesWithOriginalVersion_WhenRetrieveRequestForStudyForOriginalWithTranscoding_ThenInstancesAreReturned()
    {
        ...
        await _fileStore
            .DidNotReceive()
            .GetStreamingFileAsync(Arg.Any<long>(), Partition.Default.Name, DefaultCancellationToken);

        await _fileStore
            .Received(1)
            .GetFileAsync(Arg.Is<long>(x => x == 5), Partition.Default.Name, DefaultCancellationToken);
    }
    ...
    public async Task GivenSpecificTransferSyntax_WhenRetrieveRequestForStudy_ThenInstancesInStudyAreTranscodedSuccesfully()
    {
        ...
        _instanceStore.GetInstanceIdentifierWithPropertiesAsync(Partition.Default, _studyInstanceUid, null, null, DefaultCancellationToken).Returns(new[] { instanceMetadata });
        ...
        _fileStore.GetFileAsync(0, Partition.Default.Name, DefaultCancellationToken).Returns(streamsAndStoredFile.Value);
        _fileStore.GetFilePropertiesAsync(instanceMetadata.VersionedInstanceIdentifier.Version, Partition.Default.Name, DefaultCancellationToken).Returns(new FileProperties { ContentLength = streamsAndStoredFile.Value.Length });
