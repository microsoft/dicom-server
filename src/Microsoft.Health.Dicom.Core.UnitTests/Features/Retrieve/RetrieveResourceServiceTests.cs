    ...
    public class RetrieveResourceServiceTests
    {
        ...
        public async Task GivenNoStoredInstances_WhenRetrieveRequestForStudy_ThenNotFoundIsThrown()
        {
            ...
            await Assert.ThrowsAsync<InstanceNotFoundException>(() => _retrieveResourceService.GetInstanceResourceAsync(
                new RetrieveResourceRequest("*", KnownContentTypes.ApplicationOctetStream, _studyInstanceUid),
        ...
        public async Task GivenNoStoredInstances_WhenRetrieveRequestForSeries_ThenNotFoundIsThrown()
        {
            ...
            await Assert.ThrowsAsync<InstanceNotFoundException>(() => _retrieveResourceService.GetInstanceResourceAsync(
                new RetrieveResourceRequest("*", KnownContentTypes.ApplicationOctetStream, _studyInstanceUid, _firstSeriesInstanceUid),
                _defaultCancellationToken));
        }
        ...
        public async Task GivenNoStoredInstances_WhenRetrieveRequestForInstance_ThenNotFoundIsThrown()
        {
            ...
            await Assert.ThrowsAsync<InstanceNotFoundException>(() => _retrieveResourceService.GetInstanceResourceAsync(
                new RetrieveResourceRequest("*", KnownContentTypes.ApplicationOctetStream, _studyInstanceUid, _firstSeriesInstanceUid, _sopInstanceUid),
                _defaultCancellationToken));
        }
        ...
        public async Task GivenStoredInstancesWithoutFrames_WhenRetrieveRequestForFrame_ThenNotFoundIsThrown()
        {
            ...
            List<int> framesToRequest = new List<int> { 0 };
            ...
            RetrieveResourceRequest retrieveResourceRequest = new RetrieveResourceRequest("*", KnownContentTypes.ApplicationOctetStream, _studyInstanceUid, _firstSeriesInstanceUid, _sopInstanceUid, framesToRequest);
            _dicomFrameHandler.GetFramesResourceAsync(streamOfStoredFiles, retrieveResourceRequest.Frames, true, "*", KnownContentTypes.ApplicationOctetStream).Throws(new FrameNotFoundException());
        ...
        public async Task GivenStoredInstancesWithFrames_WhenRetrieveRequestForNonExistingFrame_ThenNotFoundIsThrown()
        {
            // Add multiple instances to validate that we evaluate the requested instance and ignore the other(s).
            List<VersionedInstanceIdentifier> versionedInstanceIdentifiers = SetupInstanceIdentifiersList(ResourceType.Frames);
            List<int> framesToRequest = new List<int> { 1, 4 };
        ...
        public async Task GivenStoredInstancesWithFrames_WhenRetrieveRequestForFrames_ThenFramesInInstanceAreRetrievedSuccesfully()
        {
            // Add multiple instances to validate that we return the requested instance and ignore the other(s).
            List<VersionedInstanceIdentifier> versionedInstanceIdentifiers = SetupInstanceIdentifiersList(ResourceType.Frames);
            List<int> framesToRequest = new List<int> { 1, 2 };
            ...
            RetrieveResourceRequest retrieveResourceRequest = new RetrieveResourceRequest("*", KnownContentTypes.ApplicationOctetStream, _studyInstanceUid, _firstSeriesInstanceUid, _sopInstanceUid, framesToRequest);
            _dicomFrameHandler.GetFramesResourceAsync(streamAndStoredFile.Value, retrieveResourceRequest.Frames, true, "*", KnownContentTypes.ApplicationOctetStream).Returns(frames);
        ...
        private List<VersionedInstanceIdentifier> SetupInstanceIdentifiersList(ResourceType resourceType)
        {
            ...
            switch (resourceType)
            {
                case ResourceType.Study:
                    ...
                    _instanceStore.GetInstanceIdentifiersInStudyAsync(_studyInstanceUid, _defaultCancellationToken).Returns(dicomInstanceIdentifiersList);
                    break;
                case ResourceType.Series:
                    dicomInstanceIdentifiersList.Add(new VersionedInstanceIdentifier(_studyInstanceUid, _firstSeriesInstanceUid, TestUidGenerator.Generate(), 0));
                    dicomInstanceIdentifiersList.Add(new VersionedInstanceIdentifier(_studyInstanceUid, _firstSeriesInstanceUid, TestUidGenerator.Generate(), 0));
                    _instanceStore.GetInstanceIdentifiersInSeriesAsync(_studyInstanceUid, _firstSeriesInstanceUid, _defaultCancellationToken).Returns(dicomInstanceIdentifiersList);
                    break;
                ...
                case ResourceType.Frames:
                    ...
                    dicomInstanceIdentifiersList.Add(new VersionedInstanceIdentifier(_studyInstanceUid, _firstSeriesInstanceUid, TestUidGenerator.Generate(), 0));
                    _instanceStore.GetInstanceIdentifierAsync(_studyInstanceUid, _firstSeriesInstanceUid, _sopInstanceUid, _defaultCancellationToken).Returns(dicomInstanceIdentifiersList.SkipLast(1));
                    break;
            }
        ...
        {
            List<DicomFile> responseFiles = responseStreams.Select(x => DicomFile.Open(x)).ToList();
