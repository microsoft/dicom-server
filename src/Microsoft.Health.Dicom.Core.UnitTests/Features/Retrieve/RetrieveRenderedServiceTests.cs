...
public class RetrieveRenderedServiceTests
{
    ...
    public async Task GivenFileSizeTooLarge_RenderForInstance_ThenNotFoundIsThrown()
    {
        ...
        _fileStore.GetFilePropertiesAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, DefaultPartition.Key, DefaultCancellationToken).Returns(new FileProperties { ContentLength = aboveMaxFileSize });
    ...
    public async Task GivenStoredInstancesWithFrames_WhenRenderRequestForNonExistingFrame_ThenNotFoundIsThrown()
    {
        ...
        _fileStore.GetFileAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, DefaultPartition.Key, DefaultCancellationToken).Returns(streamOfStoredFiles);
        _fileStore.GetFilePropertiesAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, DefaultPartition.Key, DefaultCancellationToken).Returns(new FileProperties { ContentLength = streamOfStoredFiles.Length });
    ...
    public async Task GivenStoredInstancesWithFramesJpeg_WhenRetrieveRenderedForFrames_ThenEachFrameRenderedSuccesfully()
    {
        ...
        _fileStore.GetFileAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, DefaultPartition.Key, DefaultCancellationToken).Returns(streamAndStoredFile.Value);
        _fileStore.GetFilePropertiesAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, DefaultPartition.Key, DefaultCancellationToken).Returns(new FileProperties { ContentLength = streamAndStoredFile.Value.Length });
        ...
        _fileStore.GetFileAsync(versionedInstanceIdentifiers.First().VersionedInstanceIdentifier.Version, DefaultPartition.Key, DefaultCancellationToken).Returns(streamAndStoredFileForFrame2);
