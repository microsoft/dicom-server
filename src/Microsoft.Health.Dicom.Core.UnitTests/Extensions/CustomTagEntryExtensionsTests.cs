...
namespace Microsoft.Health.Dicom.Core.UnitTests.Extensions
{
    public class CustomTagEntryExtensionsTests
    {
        ...
        public void GivenStandardTagWithoutVR_WhenFormalizing_ThenVRShouldBeFilled(string vr)
        {
            ...
            CustomTagEntry entry = CreateCustomTagEntry(tag.GetPath(), vr, CustomTagLevel.Instance, CustomTagStatus.Ready);
            CustomTagEntry normalized = entry.Normalize(CustomTagStatus.Adding);
        ...
        public void GivenTagOfLowerCase_WhenFormalizing_ThenTagShouldBeUpperCase()
        {
            ...
            CustomTagEntry entry = CreateCustomTagEntry(tag.GetPath().ToLowerInvariant(), tag.GetDefaultVR().Code, CustomTagLevel.Instance, CustomTagStatus.Ready);
            CustomTagEntry normalized = entry.Normalize(CustomTagStatus.Adding);
        ...
        {
            DicomTag tag = DicomTag.DeviceLabel;
            CustomTagEntry entry = CreateCustomTagEntry(tag.GetPath(), tag.GetDefaultVR().Code.ToLowerInvariant(), CustomTagLevel.Instance, CustomTagStatus.Ready);
            CustomTagEntry normalized = entry.Normalize(CustomTagStatus.Adding);
        ...
        {
            DicomTag tag = DicomTag.DeviceSerialNumber;
            CustomTagEntry entry = CreateCustomTagEntry(path: tag.DictionaryEntry.Keyword, tag.GetDefaultVR().Code, CustomTagLevel.Instance, CustomTagStatus.Ready);
        ...
        public static IEnumerable<object[]> GetValidCustomTagEntries()
        {
            ...
            yield return new object[] { CreateCustomTagEntry("12051003", DicomVRCode.OB, CustomTagLevel.Instance, CustomTagStatus.Ready) }; // private tag with VR
        }

        private static CustomTagEntry CreateCustomTagEntry(string path, string vr, CustomTagLevel level = CustomTagLevel.Instance, CustomTagStatus status = CustomTagStatus.Ready)
        {
