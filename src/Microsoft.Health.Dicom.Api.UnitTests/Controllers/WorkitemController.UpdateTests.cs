...
public sealed class WorkitemControllerUpdateTests
{
    private readonly WorkitemController _controller;
    private readonly IMediator _mediator;
    private readonly string _id = Guid.NewGuid().ToString();

    public WorkitemControllerUpdateTests()
    {
    ...
    public async Task GivenWorkitemInstanceUid_WhenHandlerFails_ThenReturnBadRequest()
    {
        _mediator
            .Send(
                Arg.Is<UpdateWorkitemRequest>(x => x.WorkitemInstanceUid == _id),
        ...
        ObjectResult result = await _controller.UpdateAsync(_id, new DicomDataset[1]) as ObjectResult;
