...
--    where c.object_id = OBJECT_ID('dbo.ExtendedQueryTagDateTime')
--    and c.name = 'TagValueUTC'
--    and i.name = 'IXC_ExtendedQueryTagDateTime'
--)
--BEGIN
...
--
-- DESCRIPTION
--     Altering it such that InsertDateTimeExtendedQueryTagTableType_1 can be updated.
------------------------------------------------------------------------
...
AS
    ...
    BEGIN      
        ...
        WHEN MATCHED THEN 
            UPDATE SET T.Watermark = @newWatermark, T.TagValue = S.TagValue
        WHEN NOT MATCHED THEN 
            INSERT (TagKey, TagValue, TagValueUTC, StudyKey, SeriesKey, InstanceKey, Watermark)
            VALUES(
            S.TagKey,
            S.TagValue,
            S.TagValueUTC,
