...
--
-- DESCRIPTION
--     Gets ETag for study, series, or instance.
--     For study and series, ETag is calculated using the following formula:
--          Max(Instance watermark)-Count(Instances in the study or series)
--     For instance, ETag is equals to the watermark of the respective instance.
--
-- PARAMETERS
--     @validStatus
--         * Filter criteria to search only valid instances
...
BEGIN
    ...
    SELECT CONCAT(MAX(Watermark), '-', COUNT(*)) AS ETag
    INTO #Result
    FROM Instance
    WHERE StudyInstanceUid = @StudyInstanceUid
          AND Status       = @validStatus
    ...
    BEGIN
        INSERT INTO #Result
        SELECT CONCAT(MAX(Watermark), '-', COUNT(*)) 
        FROM Instance
