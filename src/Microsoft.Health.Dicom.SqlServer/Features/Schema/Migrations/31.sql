...
CREATE TABLE dbo.Instance (
    ...
    isFirstRevision       BIT           DEFAULT 0 NOT NULL,
    isLastRevision        BIT           DEFAULT 0 NOT NULL
)
WITH (DATA_COMPRESSION = PAGE);

CREATE UNIQUE CLUSTERED INDEX IXC_Instance
    ON dbo.Instance(PartitionKey, StudyKey, SeriesKey, InstanceKey, Revision);

CREATE UNIQUE NONCLUSTERED INDEX IX_Instance_PartitionKey_StudyInstanceUid_SeriesInstanceUid_SopInstanceUid
    ON dbo.Instance(PartitionKey, StudyInstanceUid, SeriesInstanceUid, SopInstanceUid, Revision)
...
CREATE NONCLUSTERED INDEX IX_Instance_PartitionKey_Status_StudyKey_Watermark
...
CREATE NONCLUSTERED INDEX IX_Instance_PartitionKey_Status_StudyKey_SeriesKey_Watermark
...
@partitionKey INT, @studyInstanceUid VARCHAR (64), @seriesInstanceUid VARCHAR (64), @sopInstanceUid VARCHAR (64), @revision INT, @currentWatermark BIGINT, @nextWatermark BIGINT
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT OFF;
    ...
    DECLARE @transferSyntaxUid AS VARCHAR (64);
    DECLARE @hasFrameMetadata AS BIT;
    ...
    INSERT  INTO dbo.Instance (PartitionKey, StudyKey, SeriesKey, InstanceKey, StudyInstanceUid, SeriesInstanceUid, SopInstanceUid, Watermark, Status, LastStatusUpdatedDate, CreatedDate, TransferSyntaxUid, HasFrameMetadata, Revision, isFirstRevision, isLastRevision)
    VALUES                   (@partitionKey, @studyKey, @seriesKey, @instanceKey, @studyInstanceUid, @seriesInstanceUid, @sopInstanceUid, @nextWatermark, @existingStatus, @currentDate, @currentDate, @transferSyntaxUid, @hasFrameMetadata, @revision + 1, 0, 1);
    COMMIT TRANSACTION;
END
...
BEGIN
    ...
    SELECT @proposedWatermark AS ProposedWatermark
    FROM   dbo.Instance;
END
...
@partitionKey INT, @studyInstanceUid VARCHAR (64), @patientId NVARCHAR (64), @patientName NVARCHAR (325)=NULL, @referringPhysicianName NVARCHAR (325)=NULL, @studyDate DATE=NULL, @studyDescription NVARCHAR (64)=NULL, @accessionNumber NVARCHAR (64)=NULL
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT OFF;
