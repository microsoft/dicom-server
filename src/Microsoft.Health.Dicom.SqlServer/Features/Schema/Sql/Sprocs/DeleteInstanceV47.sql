...
BEGIN
    ...
    DELETE  dbo.Instance
        OUTPUT deleted.PartitionKey, deleted.StudyInstanceUid, deleted.SeriesInstanceUid, deleted.SopInstanceUid, deleted.Status, deleted.Watermark, deleted.InstanceKey, FP.FilePath, FP.ETag
        INTO @deletedInstances
    LEFT OUTER JOIN dbo.FileProperty as FP
        ON InstanceKey = FP.InstanceKey
        AND Watermark = dbo.FileProperty.Watermark
    WHERE   PartitionKey = @partitionKey
        AND     StudyInstanceUid = @studyInstanceUid
        AND     SeriesInstanceUid = ISNULL(@seriesInstanceUid, SeriesInstanceUid)
        AND     SopInstanceUid = ISNULL(@sopInstanceUid, SopInstanceUid)
    ...
    AND DI.Watermark = dbo.FileProperty.Watermark
    ...
    (PartitionKey, StudyInstanceUid, SeriesInstanceUid, SopInstanceUid, Watermark, DeletedDateTime, RetryCount, CleanupAfter, FilePath, ETag)
    SELECT PartitionKey, StudyInstanceUid, SeriesInstanceUid, SopInstanceUid, Watermark, @deletedDate, 0 , @cleanupAfter, FilePath, ETag
