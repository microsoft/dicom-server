...
internal sealed class SqlIndexDataStore : IIndexDataStore
{
    ...
    public async Task UpdateFrameDataAsync(int partitionKey, IReadOnlyCollection<long> versions, bool hasFrameMetadata, CancellationToken cancellationToken = default)
    {
        ISqlIndexDataStore store = await _cache.GetAsync(cancellationToken: cancellationToken);
