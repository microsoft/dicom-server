...
namespace Microsoft.Health.Dicom.SqlServer.Features.Retrieve
{
    internal sealed class SqlInstanceStore : IInstanceStore
    {
        ...
        public async Task<IEnumerable<VersionedInstanceIdentifier>> GetInstanceIdentifierAsync(string partitionId, string studyInstanceUid, string seriesInstanceUid, string sopInstanceUid, CancellationToken cancellationToken = default)
        {
            ISqlInstanceStore store = await _cache.GetAsync(cancellationToken: cancellationToken);
            return await store.GetInstanceIdentifierAsync(partitionId, studyInstanceUid, seriesInstanceUid, sopInstanceUid, cancellationToken);
        }
        ...
        public async Task<IEnumerable<VersionedInstanceIdentifier>> GetInstanceIdentifiersInSeriesAsync(string partitionId, string studyInstanceUid, string seriesInstanceUid, CancellationToken cancellationToken = default)
        {
            ISqlInstanceStore store = await _cache.GetAsync(cancellationToken: cancellationToken);
            return await store.GetInstanceIdentifiersInSeriesAsync(partitionId, studyInstanceUid, seriesInstanceUid, cancellationToken);
        }

        public async Task<IEnumerable<VersionedInstanceIdentifier>> GetInstanceIdentifiersInStudyAsync(string partitionId, string studyInstanceUid, CancellationToken cancellationToken = default)
        {
            ISqlInstanceStore store = await _cache.GetAsync(cancellationToken: cancellationToken);
            return await store.GetInstanceIdentifiersInStudyAsync(partitionId, studyInstanceUid, cancellationToken);
        }
