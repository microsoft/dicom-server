...
public partial class RetrieveTransactionResourceTests : IClassFixture<HttpIntegrationTestFixture<Startup>>, IAsyncLifetime
{
    ...
    private async Task<(InstanceIdentifier, DicomFile)> CreateAndStoreDicomFile(int numberOfFrames = 0)
    {
        ...
        var dicomInstance = dicomFile.Dataset.ToInstanceIdentifier(DefaultPartition.PartitionEntry);
    ...
    {
        InstanceIdentifier newId = new InstanceIdentifier(TestUidGenerator.Generate(), TestUidGenerator.Generate(), TestUidGenerator.Generate(), PartitionEntry.Default);
