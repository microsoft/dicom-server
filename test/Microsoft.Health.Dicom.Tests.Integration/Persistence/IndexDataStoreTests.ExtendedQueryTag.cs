...
namespace Microsoft.Health.Dicom.Tests.Integration.Persistence
{
    ...
    public partial class IndexDataStoreTests : IClassFixture<SqlDataStoreTestsFixture>
    {
        ...
        internal async Task GivenDicomInstanceWithDifferentTypeOfExtendedQueryTags_WhenStore_ThenTheyShouldBeStoredInProperTable(ExtendedQueryTagDataType dataType, DicomElement element, object expectedValue)
        {
            ...
            try
            {
                long watermark = await CreateInstanceIndexAsync(null, dataset, new QueryTag[] { queryTag });
        ...
        private async Task<QueryTag> AddExtendedQueryTag(AddExtendedQueryTagEntry extendedQueryTagEntry)
        {
        ...
        private async Task ValidateUpdateExistingExtendedQueryTagIndexData(QueryTagLevel level)
        {
            ...
            try
            {
                ...
                await CreateInstanceIndexAsync(null, dataset, new QueryTag[] { queryTag });
        ...
        private async Task<Instance> StoreInstanceWithDifferentTagValues(string studyInstanceUid, string seriesInstanceUid, string sopInstanceUid, int index, IReadOnlyList<QueryTag> queryTags)
        {
            ...
            await _indexDataStore.EndCreateInstanceIndexAsync(null, dataset, watermark, queryTags);
        ...
        private async Task<long> CreateInstanceIndexAsync(string partitionId, DicomDataset dicomDataset, IReadOnlyList<QueryTag> queryTags)
        {
            long watermark = await _indexDataStore.BeginCreateInstanceIndexAsync(partitionId, dicomDataset, queryTags);
            await _indexDataStore.EndCreateInstanceIndexAsync(partitionId, dicomDataset, watermark, queryTags);
            return watermark;
        }
    }
}
